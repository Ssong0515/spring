...
// WebSecurityConfig: 스프링 시큐리티 설정을 위한 클래스, 사용자가 원하는 클래스명으로 생성하고, @Configuration, @EnableWebSecurity 어노테이션을 추가하여 사용
// 예전에는 WebSecurityConfigurerAdapter 클래스를 상속받아 설정 했지만, Spring Security 5 (스프링부터 2.7 이후)부터는 WebSecurityConfigurerAdapter를 상속받지 않고 SecurityFilterChain을 사용하는 방식으로 변경됨

@Configuration
// @EnableWebSecurity: 스프링 시큐리티를 활성화 하고 사용자 설정을 사용할 수 있도록 하는 어노테이션
@EnableWebSecurity
...
public class WebSecurityConfig {
    ...
    // WebSecurityCustomizer는 함수형 인터페이스로 configure() 메서드를 오버라이딩 하여 설정
    ...
    public WebSecurityCustomizer configure(){
        ...
        // toH2Console(): H2 데이터베이스의 콘솔의 주소를 의미
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        return http
                ...
                .authorizeHttpRequests(auth -> auth
                        ...
                        .anyRequest().authenticated() // '그 외 모든 요청은 모두 인증 받아야 한다'는 설정
                )
                // Web의 Form을 사용하여 로그인 한다는 설정
