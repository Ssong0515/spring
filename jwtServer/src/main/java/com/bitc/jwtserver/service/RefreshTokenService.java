...
//    데이터베이스에 생성된 리플레시 토큰 엔티티 타입의 데이터를 저장하고 반환
...
//  데이터베이스에 매개변수로 받은 리플레시 토큰이 저장되어 있는지 확인
  public Optional<MemberEntity> findMemberByToken(String token) {
//    매개변수로 받은 리플레시 토큰과 동일한 값이 데이터베이스에 있는 확안
    return refreshTokenRepository.findByRefreshToken(token)
//        검색한 리플레시 토큰의 만료 기간 확인
