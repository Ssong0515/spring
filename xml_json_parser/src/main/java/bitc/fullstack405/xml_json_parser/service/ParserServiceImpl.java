...
//    jaxb가 자바 9버전부터 기본 라이브러리에서 제외 됨

//    Marshal(마샬): 자바 클래스를 xml 데이터로 변환
//    UnMarshal(언마샬): xml 데이터를 자바 클래스 타입의 객체로 변환

    @Override
    public List<FullDataItemDTO> getItemListFile(String fileName) throws JAXBException {

//        JAXB 라이브러리 사용 선언
//        JAXB는 싱글톤 방식으로 되어 있어서 아래처럼 선언 해줌
...
//        기존에 제공된 xml 데이터를 기반으로 FullDataResponseDTO 클래스의 객체를 생성하므로
//        xml 데이터를 파싱하여 가져온 데이터를 FullDataResponseDTO 클래스 타입의 객체에 타입 변환하여 저장함
//        unmarshal(): 언마샬을 수행하는 메서드, 매개변수로 파일이나 URL을 받음 -> Object로 반환
...
//            xml 데이터를 파싱하기 위해서 JAXB 라이브러리 사용
            JAXBContext jc = JAXBContext.newInstance(FullDataResponseDTO.class);
//            xml 데이터를 자바 클래스 타입으로 변환하는 객체 생성
            Unmarshaller um = jc.createUnmarshaller();

//            xml 데이터를 자바 클래스 타입으로 변환하는unmarshal()
...
//            itemList = items.getItemList();
    ...
    @Override
    public List<DailyBoxOfficeDTO> getDailyBoxOfficeList(String serviceUrl) throws Exception {

        List<DailyBoxOfficeDTO> dailyBoxOfficeList = new ArrayList<>();
...
//        그러나 json 파싱 라이브러리에는 네트워크 데이터 처리 부분이 없기 때문에
//        직접 InputStreamReader를 통하여 네트워크에서 데이터를 가져오는 부분과 가져온 데이터를
//        처리하는 부분이 필요함

//            지정한 서버에서 json 데이터를 가져옴, 가져온 데이터를 BufferedReader에 저장함
            reader = new BufferedReader(new InputStreamReader(urlConn.getInputStream()));

//            네트워크를 통해서 전달된 데이터가 너무 길기 때문에 하나의 문자열로 합쳐주는 클래스(StringBuilder/StringBuffer 둘중에 하나 쓰면 됨)
            StringBuilder sb = new StringBuilder();
//            json 데이터가 저장될 변수
            String line;

//            BufferedReader에 저장된 데이터를 한 라인씩 가져와 변수 line에 저장
