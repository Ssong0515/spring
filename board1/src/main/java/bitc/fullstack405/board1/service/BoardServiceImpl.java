...
//    @Autowired 어노테이션을 사용하여 스프링 프레임워크가 자동으로 객체를 생성하고 관리함
    ...
    @Autowired
    private bitc.fullstack405.board1.utils.FileUtils fileUtils;
...
//     사용자가 입력한 Form 데이터와 업로드 된 파일 정보를 매개변수로 받아서 사용
    @Override
    public void insertBoard(BoardDTO board, MultipartHttpServletRequest multipart) throws Exception {
//      매퍼를 사용하여 DB에 게시물 등록
        boardMapper.insertBoard(board);

//        업로드 된 파일 정보를 모두 가져옴, 해당 정보를 가지고 DB의 t_files 테이블에 데이터를 저장 할 수 있음
...
// 파일 정보가 서버로 업로드 되었는지 확인
////        ObjectUtils: 스프링 프레임워크에서 제공하는 클래스/ 객체가 비었는지 확인 해줌
//        if (ObjectUtils.isEmpty(multipart) == false) {
////            getFileNames(): 업로드 된 파일 정보에서 모든 파일 이름을 출력
//            Iterator<String> iterator = multipart.getFileNames();
//            String name;
////            Collection<MultipartFile> iterator = multipart.getFileMap().values();
