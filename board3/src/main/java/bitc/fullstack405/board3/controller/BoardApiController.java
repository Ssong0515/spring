...
// ajax 통시 시 rest api 방식으로 데이터를 전송하는 컨트롤러를 따로 만들어서 사용하면 편리함
// 리액트 사용 시, 프론트엔드 부분과 백엔드 부분이 완전히 분리되어 있기 때문에 데이터를 주고 받기 위해서 aja 통신이 필요하x

// RESTful API: REST 방식과 동일한 형태로 HTTP 프로토콜을 사용하지만, 클라이언트로 전달하는 데이터가 View를 제외한 순수 데이터만 전달하는 방식
// 예전에는 클라리언트가 컴퓨터 밖에 없었기 때문에 웹 브라우저 혹은 전용 프로그램이 클라이언트의 기능을 모두 담당했음
// 서버의 기능의 웹 서버의 기능만 주로 사용되었기 때문에 클라이언트로 전달하는 데이터가 View와 Model 데이터 모두 필요한 형태였음
// 현재는 클라이언트가 다양한 디바이스로 동작하기 때문에 View 부분이 반드시 웹 브라우저일 필요가 없디 때문에 데이터만 전달하는 방식으로 변경되었음
...
public class BoardApiController {
    ...
    // @RequestParam: 서버 주소(URL)에 데이터를 포함하여 전달하는 방식
    // 게시판 글 등록 API
